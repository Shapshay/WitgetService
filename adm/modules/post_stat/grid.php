...
$result = mysql_query("SELECT COUNT(*) AS count FROM posts");
...
$SQL = "SELECT id, theme, date, otpr, otkr FROM posts  ORDER BY date DESC LIMIT $start , $limit";
//echo $SQL;
...
while($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
	...
	$responce->rows[$i]['cell']=array($row['id'],$row['theme'],$row['date'],$row['otpr'],$row['otkr']);
	$i++;
} 
