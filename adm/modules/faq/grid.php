...
$result = mysql_query("SELECT COUNT(*) AS count FROM faq");
...
$SQL = "SELECT id, name, date, question, view FROM faq WHERE page_id = ".$_GET['ch']." ORDER BY date DESC LIMIT $start , $limit";
//echo $SQL;
...
while($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
	...
	$responce->rows[$i]['cell']=array("",$row['id'],$row['date'],$row['name'],$row['question'],$row['view']);
	$i++;
} 
