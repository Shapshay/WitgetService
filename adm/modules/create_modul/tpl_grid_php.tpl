...
$result = mysql_query("SELECT COUNT(*) AS count FROM {MODUL_TABLE_NAME}");
...
$SQL = "SELECT id, {MODUL_GRID_PHP_FIELDS1} FROM {MODUL_TABLE_NAME} WHERE page_id = ".$_GET['ch']." LIMIT $start , $limit";
//echo $SQL;
...
while($row = mysql_fetch_array($result,MYSQL_ASSOC)) {
	...
	$responce->rows[$i]['cell']=array("",$row['id'],{MODUL_GRID_PHP_FIELDS2});
	$i++;
} 
