<?php
	# SETTINGS #############################################################################
	$moduleName = "w_contact5";
	$prefix = "./modules/".$moduleName."/";
	$tpl->define(array(
			$moduleName => $prefix . $moduleName.".tpl",
			$moduleName . "main" => $prefix . "main.tpl",
			$moduleName . "html" => $prefix . "html.tpl",
			$moduleName . "script" => $prefix . "w_contact5.js",
	));
	# MAIN #################################################################################
	//echo "Y";
	if(!isset($_GET['u'])){
	...
	if(db_num_rows($result) > 0){
		...
		if(!isset($_GET['u'])){
			...
			$tpl->assign("WDT_ACTION", $url);
			if($copyright==0){
				$tpl->assign("WDT_COPYRIGHT", '<a href=\"http://witgetservice.com/\" target=\"_blank\" style=\"text-decoration:none;\">All rights reserved © WitgetService.com, 2016</a>');
			}
			else{
				$tpl->assign("WDT_COPYRIGHT", '');
			}
