<?php
	...
	$tpl->define(array(
			...
			$moduleName . "s2_row" => $prefix . "s2_row.tpl",
	));
	# MAIN #################################################################################
	...
	if(!isset($_GET['u'])){
		...
		if(db_num_rows($result) > 0){
			...
			$sql2 = "SELECT * FROM stat_contact WHERE u_id = ".U_BASE_ID." AND w_id = '".$row['id']."' ORDER BY date DESC";
			...
			if(db_num_rows($result3) > 0){
				while ($row3 = db_fetch_array($result3)) { 
					...
					$tpl->parse("STAT2_ROWS", "." .$moduleName. "s2_row");
					$i++;
				}
				
			}
			else{
				$tpl->assign("STAT2_ROWS", '<tr><td colspan="6" style="color:#FF0000" align="center">Данных необнаружено !</td></tr>');
			}
