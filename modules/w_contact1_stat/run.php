<?php
	...
	if(isset($_POST['wdt_btn_save'])){
		...
		if(preg_match ($pattern , $_POST['wdt_email'])){
			...
			if(db_num_rows($result) > 0){
				...
				if(setStatContactEmailID($row['id'], $U_BASE_ID, $_POST['wdt_email'])==0){
					$sql = "INSERT INTO stat_contact SET 
								w_id = '".$row['id']."',
								u_id = '".$U_BASE_ID."',
								email = '".$_POST['wdt_email']."',
								date = NOW()";
					db_query($sql);
				}
				/*
	...
	if(!isset($_GET['u'])){
		...
		if(db_num_rows($result) > 0){
			...
			$sql2 = "SELECT email FROM stat_contact WHERE u_id = ".U_BASE_ID." AND w_id = '".$row['id']."'";
			$result2 = db_query($sql2);
			if(db_num_rows($result2) > 0){
				$i = 1;
				while ($row2 = db_fetch_array($result2)) { 
					$tpl->assign("STAT_NUM", $i);
					$tpl->assign("STAT_EMAIL", $row2['email']);
				...
				$tpl->assign("STAT_ROWS", '<tr><td colspan="2" style="color:#FF0000" align="center">Данных необнаружено !</td></tr>');
			}
