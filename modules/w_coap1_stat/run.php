<?php
	...
	if(!isset($_GET['u'])){
		...
		if(db_num_rows($result) > 0){
			...
			$sql2 = "SELECT * FROM coap WHERE u_id = ".U_BASE_ID." AND w_id = '".$row['id']."' AND send = 1 ORDER BY date DESC";
			...
			if(db_num_rows($result2) > 0){
				...
				while ($row2 = db_fetch_array($result2)) { 
					...
					$tpl->assign("STAT_NAME", $row2['name']);
					$tpl->assign("STAT_EMAIL", $row2['email']);
					$tpl->assign("STAT_PHONE", $row2['phone']);
					$tpl->assign("STAT_GN", $row2['gn']);
					$tpl->assign("STAT_PN", $row2['pn']);
				...
				$tpl->assign("STAT_ROWS", '<tr><td colspan="7" style="color:#FF0000" align="center">Данных необнаружено !</td></tr>');
			}
