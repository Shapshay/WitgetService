<?php
	# SETTINGS #############################################################################
	$moduleName = "w_action2";
	$prefix = "./modules/".$moduleName."/";
	$tpl->define(array(
			$moduleName => $prefix . $moduleName.".tpl",
			$moduleName . "main" => $prefix . "main.tpl",
			$moduleName . "html" => $prefix . "html.tpl",
			$moduleName . "script" => $prefix . "w_action2.js",
	));
	# MAIN #################################################################################
	//echo "Y";
	if(!isset($_GET['u'])){
	...
	if(db_num_rows($result) > 0){
		...
		if(!isset($_GET['u'])){
			...
			$tpl->assign("WDT_ACTION", $url);
			if($copyright==0){
				$tpl->assign("WDT_COPYRIGHT", '<br /><a href=\"http://witgetservice.com/\" target=\"_blank\" style=\"font-size:10px;color:#ffffff;text-decoration:none;\">WitgetService.com</a>');
			}
			else{
				$tpl->assign("WDT_COPYRIGHT", '');
			}
