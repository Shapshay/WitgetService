<?php
	...
	$tpl->define(array(
			...
			$moduleName . "script" => $prefix . "w_soc1.js",
	));
	# MAIN #################################################################################
	//echo "Y";
	if(!isset($_GET['u'])){
	...
	if(db_num_rows($result) > 0){
		...
		if(!isset($_GET['u'])){
			...
			$tpl->assign("WDT_ACTION", $url);
			if($copyright==0){
				$tpl->assign("WDT_COPYRIGHT", '<a href=\"http://witgetservice.com/\" target=\"_blank\" style=\"font-size:10px;color:#000000;text-decoration:none;\">WitgetService.com</a>');
			}
			else{
				$tpl->assign("WDT_COPYRIGHT", '');
			}
