<?php
			...
			if($key!='btn_wdt_save'&&$key!='video'){
				...
				if($key=='video'){
					...
					if(preg_match($patern, $v, $matches)){
						$wdt_opt_arr[$key] = '//www.youtube.com/embed/'.str_replace("v=","",$matches[0]);
						//echo str_replace("v=","",$matches[0]);
					}
					else{
						$patern = '/\/embed\/\S{11}/i';
						if(preg_match($patern, $v, $matches)){
							$wdt_opt_arr[$key] = '//www.youtube.com/embed/'.str_replace("/embed/","",$matches[0]);
							//echo str_replace("/embed/","",$matches[0]);
						}
						else{
							$wdt_opt_arr[$key] = $_POST['old_video'];
						}
					}
				}
			}
	...
	if(db_num_rows($result) > 0){
		...
		$tpl->assign("WDT_EDT_IMG", $options->img);
		$tpl->assign("WDT_EDT_URL", $options->url);
		$tpl->assign("WDT_EDT_VIDEO", $options->video);
